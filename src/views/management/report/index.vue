<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="roleList" border>
      ...
      <el-table-column label="最近一次体检套餐" align="center" prop="packageName"/>
      <el-table-column label="最近一次体检日期" align="center" prop="reportTime"/>
...
<script>
import {getList,managExport} from "@/api/management/report";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/examReport/import"
      },
    };
  },
  created() {
  ...
  methods: {
    /** 下载模板操作 */
    importTemplate() {
      let url = process.env.VUE_APP_BASE_API+'/model/download/examReport'
      window.open(url)
    },
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = "患者报告导入";
    ...
    /** 查看详情 */
    handleDetail(row) {
      const {reportCode} = row
      this.$router.push({path: '/management/rep/detail/' + reportCode})
    },
  }
};
</script>
